...
program
	...
	.option(
		"--migrate",
		"Migrate an existing Angular project to NG Fortress workflow",
	)
	.argument(
		"[project-name]",
		"Name of the project to create (or directory of existing app)",
	)
	.parse(process.argv);

async function findAngularApps(dir: string) {
	const apps = [];
	...
	for (const item of items) {
		if (
			...
			if (
				fs.existsSync(path.join(fullPath, "angular.json")) ||
				...
				for (const subItem of subItems) {
					if (subItem.isDirectory()) {
						...
						if (
							fs.existsSync(path.join(subFullPath, "angular.json")) ||
							fs.existsSync(path.join(subFullPath, "project.json"))
						) {
...
async function main() {
	...
	if (isMigrate) {
		...
		if (projectName) {
			...
			if (
				...
				if (apps.length === 0) {
					console.error(
						chalk.red(
							"No Angular applications found in this monorepo to migrate.",
						),
					);
					process.exit(1);
				}
				...
				console.error(
					chalk.red(
						"Unable to determine project to migrate. Please run inside an Angular project or specify the path.",
					),
				);
				process.exit(1);
			}
		...
		if (
			...
			console.error(
				chalk.red(
					`The specified path does not contain a valid Angular application: ${projectPath}`,
				),
			);
			process.exit(1);
		}
