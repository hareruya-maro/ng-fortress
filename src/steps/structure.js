...
export async function setupDirStructureAndAgents(projectPath) {
...
`;
  fs.writeFileSync(path.join(appDir, 'app.component.ts'), strictAppComponentContent);
...
`;
  fs.writeFileSync(path.join(appDir, 'app.component.spec.ts'), strictAppSpecContent);

  // 2.5 Fix main.ts bootstrap reference
  const mainTsPath = path.join(projectPath, 'src/main.ts');
  if (fs.existsSync(mainTsPath)) {
    let mainContent = fs.readFileSync(mainTsPath, 'utf8');
    mainContent = mainContent.replace(/import\s*{\s*App\s*}\s*from\s*['"]\.\/app\/app['"];?/, "import { AppComponent } from './app/app.component';\n");
    mainContent = mainContent.replace(/bootstrapApplication\(App/, 'bootstrapApplication(AppComponent');
    fs.writeFileSync(mainTsPath, mainContent);
  }

  // 3. Create Agent instructions
  console.log(chalk.green('🤖 [Step 3.5] Generating AI Agent Contexts (AGENTS.md & skills)...'));
...
`;
  fs.writeFileSync(path.join(projectPath, 'AGENTS.md'), agentsMdContent);

  const agentSkillsDir = path.join(projectPath, '.agent', 'skills');
...
`);
  ...
  fs.writeFileSync(path.join(agentSkillsDir, '05-workflow-and-git.md'), `# Workflow & Git Commits
