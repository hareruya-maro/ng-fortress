...
export async function setupZoneless(projectPath) {
    ...
    if (fs.existsSync(appConfigPath)) {
        ...
        content = content.replace(/import\s*{\s*([^}]*?)\s*}\s*from\s*'@angular\/core';/, (match, group1) => {
            const imports = group1.split(',').map(s => s.trim()).filter(s => s !== 'provideZoneChangeDetection');
